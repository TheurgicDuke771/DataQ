...
#: The four points along the transport where delivery can fail, in the order they're attempted.
...
class OtpMailer:
    ...
    def _resolve_password(self) -> str:
        ...
        if not (s.auth_email_smtp_host and s.auth_email_username and s.auth_email_from):
            # Unreachable behind `otp_auth_configured` + the startup validator for `send_code`'s
            # sign-in caller; `send_preflight`'s admin caller has no such upstream gate.
    ...
    def _deliver(self, message: EmailMessage, password: str) -> None:
        ...
        if tls_mode != "none":
            # `cafile=None` behaves exactly like the pre-#1146 call (system trust store only).
            ca_bundle = (s.auth_email_ca_bundle or "").strip() or None
            context = ssl.create_default_context(cafile=ca_bundle)
        else:
            # No boot-time signal exists for "someone is about to point this at a real relay" — the
            # validator only ever sees config, not intent.
            log.warning(
    ...
    def send_code(self, *, to: str, code: str, expires_in_minutes: int) -> None:
        ...
        try:
            ...
            # Broad classification on purpose: smtplib raises OSError/socket.timeout/ ssl.SSLError
            # as well as its own SMTPException family.
            log.error(
