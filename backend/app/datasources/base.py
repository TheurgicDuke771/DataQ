...
# datasource seam because *multiple* producers write those two columns — the GX
# path (`gx_runner._extract_sample_failures` / `gx_runner._bounded_observed_value`)
# and the comparison path (`comparison.SAMPLE_LIMIT`) — and the read path
# (`run_service`) re-applies it so already-persisted oversized rows stop shipping
# unbounded payloads too. Keeping a single constant means raising it can't leave
# result paths disagreeing. 20 matches GX's own `partial_unexpected_count` default
# and what the run-detail UI renders (`RunDetail.tsx` `MAX_SAMPLE_ROWS`).
SAMPLE_ROW_CAP = 20
