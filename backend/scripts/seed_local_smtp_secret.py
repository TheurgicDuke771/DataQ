...
def main() -> int:
    ...
    try:
        ...
        print(f"secret store unavailable while checking {name!r}: {exc}", file=sys.stderr)
        return 1
    else:
        print(f"SMTP password secret {name!r} already set — left untouched.")
        return 0

    try:
        store.set(name, secrets.token_urlsafe(_PASSWORD_BYTES))
    except (SecretWriteError, SecretStoreUnavailableError) as exc:
        print(f"could not write SMTP password secret {name!r}: {exc}", file=sys.stderr)
        return 1
    print(f"SMTP password secret {name!r} provisioned for the bundled mail catcher.")
    return 0
