...
def test_akv_store_get_wraps_sdk_exception(
    ...
    monkeypatch.setattr(store, "_client_lazy", lambda: fake_client)
    # NOT SecretNotFoundError: callers degrade on that ("no webhook configured"), so a Key Vault
    # outage would render as "nothing is set" across the workspace.
...
class _StubSecretClient:
    ...
    def __init__(self, *, vault_url: str, credential: object) -> None:
        ...
        self.closed = False
        # Listing surface for the orphan sweep (#1059).
...
# ── startup validation (review finding: the "raises at startup" claim was false) ── These build the
# REAL `Settings` model, not the `SimpleNamespace` the factory tests use.
...
def test_delete_stays_fail_soft_when_the_login_fails() -> None:
    """`delete`'s fail-soft contract is depended on BY NAME at three call sites — the post-commit
    cleanup in connection_service, the #1059 purge loop, and notification_service. A login
    failure arrives as `SecretStoreUnavailableError`, not an httpx error, so it would sail past
    the old handler and 500 a delete whose row is already committed.
    """
