"""POST /llm/check_suggestions (ADR 0042, #1513): gates, dispatch, failure shapes."""
...
def _body(suite: Suite) -> dict[str, Any]:
    return {"suite_id": str(suite.id)}


def test_suggestion_queues_and_dispatches(
    client: TestClient,
    db_session: Any,
    ...
    resp = client.post("/api/v1/llm/check_suggestions", json=_body(suite), headers=headers)
    ...
    assert poll.json()["kind"] == "check_suggestion"


def test_suggestion_requires_edit_grant(
    client: TestClient,
    db_session: Any,
    store: FakeSecretStore,
    dispatched: list[str],
    ...
    resp = client.post("/api/v1/llm/check_suggestions", json=_body(suite), headers=stranger_headers)
    assert resp.status_code in (403, 404)
    assert dispatched == []


def test_suggestion_refuses_non_sql_datasource(
    client: TestClient,
    db_session: Any,
    store: FakeSecretStore,
    dispatched: list[str],
...
def test_suggestion_unconfigured_is_409_not_500(
    client: TestClient,
    db_session: Any,
    dispatched: list[str],
    as_role: Callable[..., tuple[Any, dict[str, str]]],
...
def test_suggestion_targetless_suite_is_a_synchronous_422(
    client: TestClient,
    db_session: Any,
    store: FakeSecretStore,
    dispatched: list[str],
