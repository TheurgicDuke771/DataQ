...
def test_implicit_mode_classifies_a_handshake_failure_as_connect_not_tls(
    ...
    """`SMTP_SSL`'s constructor does the TCP connect AND the TLS handshake together — there is no
    separate post-connect hook to fail on its own, so a certificate failure here is genuinely
    the 'connect' stage, never 'tls' (that stage is reachable only via the `starttls` branch).
    """
...
def test_a_ca_bundle_reaches_the_real_ssl_context(
    ...
    # `get_ca_certs()`'s dict shape is loosely typed upstream; a substring check on its repr is a
    # stable.
...
def test_no_ca_bundle_configured_uses_the_ordinary_system_default_context(
    ...
    """Unset `AUTH_EMAIL_CA_BUNDLE` must build the exact same context shape as before #1146:
    `cafile=None` falls through to `create_default_context`'s own `load_default_certs()` branch.
    """
