...
def test_historical_check_engine_resolves_the_version_in_effect(db_session: Any) -> None:
    # #1869 review: a result must report the engine that actually produced it, not
    # whatever the check has since been re-pointed to.
    _suite, check = _suite_and_check(db_session)
    _version(
        db_session,
        check,
    ...
    engines = run_service.historical_check_engine(
        db_session, [before_edit, after_edit], {check.id: check}
    )

    assert engines[before_edit.id] == "dmf"
    assert engines[after_edit.id] == "gx"


def test_historical_check_engine_falls_back_to_the_live_check_with_no_version_history(
    db_session: Any,
) -> None:
    _suite, check = _suite_and_check(db_session)
    result = _result(db_session, check, created_at=_T0)
