...
def test_the_migration_engine_keeps_the_portable_lock_timeout_option() -> None:
    """`options` (the lock_timeout GUC) must still reach the engine for a psycopg driver — but as a
    KEYWORD ARGUMENT of the `**psycopg_connect_args(...)` unpack, not as a literal
    `connect_args` dict key.
    """
