...
def test_bare_postgresql_scheme_is_treated_as_psycopg() -> None:
    """A bare `postgresql://` URL resolves to SQLAlchemy's own DEFAULT DBAPI choice at
    `create_engine` time — and that default IS psycopg2 (`make_url(...).get_dialect().driver ==
    "psycopg2"`).
    """
    result = psycopg_connect_args(
...
def test_options_is_guarded_exactly_like_connect_timeout_and_keepalives() -> None:
    """`options` (the lock_timeout GUC, `-c lock_timeout=...`) is NOT portable across every driver
    — asyncpg has no `options` connect kwarg at all (it uses `server_settings` instead) — so it
    must flow through this same guard as a driver-only kwarg, not stay an unconditional dict key
    at the call site.
    """
