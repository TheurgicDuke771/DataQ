...
def upgrade() -> None:
    # ── checks: monitor-kind discriminator (ADR 0012/0014) ── NOT NULL with a server_default so
    # existing rows backfill to 'expectation' without a separate data step.
    op.add_column(
        "checks",
    ...
    # ── results.status: retarget to severity tiers (ADR 0005) ── Update existing rows to the new
    # vocabulary BEFORE swapping the CHECK, so the constraint never has to reject pre-existing data.
