...
// The app shell loads, dev-bypass resolves an identity (no login wall), and the root redirects to
// the Dashboard.
test('app shell loads under dev-bypass and redirects to Dashboard', async ({ page }) => {
  ...
  // The header brand (yin-yang logo + wordmark) + the resolved identity in the account menu (proves
  // auth resolved, so every /api call carries the user).
