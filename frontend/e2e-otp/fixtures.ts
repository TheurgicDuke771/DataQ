...
export interface SignIn {
  ...
  /** The whole flow, ending inside the app. */
...
function makeSignIn(page: Page): SignIn {
  ...
  return {
    ...
    complete: async (email: string, opts?: { dismissProfilePrompt?: boolean }) => {
      ...
      // Every fresh OTP signup starts with display_name: NULL.
      const skipPrompt = page.getByRole('button', { name: 'Skip for now' });
      await skipPrompt.waitFor({ state: 'visible' });
      // NOTE: clicking "Skip for now" writes `dataq:profileCompletionPrompt:skipped` to
      // sessionStorage (by design — see the component).
      await expect(async () => {
