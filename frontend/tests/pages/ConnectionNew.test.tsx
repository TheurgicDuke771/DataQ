...
describe('ConnectionNew', () => {
  ...
  it('submits inventory_sync=true by default, on by default (asset-first, 2026-09)', async () => {
    const user = userEvent.setup();
    mockCreate.mockResolvedValue({
      id: 'c1',
      name: 'sf-inv',
    ...
    // The toggle renders as an antd Switch (role=switch), default ON.
  ...
  it('submits inventory_sync=false when the toggle is switched off', async () => {
    const user = userEvent.setup();
    mockCreate.mockResolvedValue({
      id: 'c1',
      name: 'sf-inv',
    ...
    await user.click(screen.getByRole('button', { name: 'Create' }));
    await waitFor(() => expect(mockCreate).toHaveBeenCalledTimes(1));
    expect(mockCreate.mock.calls[0][0].config).toMatchObject({ inventory_sync: false });
  });
