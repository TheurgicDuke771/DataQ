...
**Current week:** Week 4 — Connection manager UI + check editor UI (frontend). **Weeks 1–3 complete** (exit gates met); Week 4 in progress (~25/26 — only the admin page remains, backend-blocked on a list-all endpoint).
...
**Next milestone:** finish Week 4 frontend — **dry-run preview** (PR-D1), **column-profiler panel** (PR-D2), **suite export/import UI** (PR-E1), **suite sharing panel** (PR-E2 backend + PR-E3 UI; gated on `SuiteRead.my_permission`), the datasource-shaped **run-target editor** on the suite drawer (PR #246; `SuiteDrawer` + `suiteTarget.ts` — makes UI-created suites runnable, closing the flat-file + UC target inputs), the **manual Run button** (PR #245; edit-gated `POST /suites/{id}/run`), the **Suite Triggers panel** (PR #247; `TriggersPanel` + `api/triggerBindings.ts` over `trigger_bindings` CRUD — closes [#216](https://github.com/TheurgicDuke771/DataQ/issues/216)), and the **Monaco custom-SQL editor** (backend [#258](https://github.com/TheurgicDuke771/DataQ/pull/258) + frontend [#259](https://github.com/TheurgicDuke771/DataQ/pull/259) — custom-SQL is a GX `UnexpectedRowsExpectation` under `kind='expectation'`, **ADR 0019**; this revised the old "needs a custom-SQL check kind" assumption) and the **check version-history drawer** (#280; backend [#281](https://github.com/TheurgicDuke771/DataQ/pull/281) `check_versions` snapshot table + view-gated read API, frontend [#282](https://github.com/TheurgicDuke771/DataQ/pull/282) `CheckHistoryDrawer` — view-only, restore deferred to [#283](https://github.com/TheurgicDuke771/DataQ/issues/283)) all shipped; **remaining:** the **admin page** (all suites/users/access overview — still **backend-blocked** on a list-all endpoint).
**Active blockers:** none. Open follow-ups: [#147](https://github.com/TheurgicDuke771/DataQ/issues/147) (profiler cleanup), Week-4 frontend nits [#192](https://github.com/TheurgicDuke771/DataQ/issues/192)/[#197](https://github.com/TheurgicDuke771/DataQ/issues/197)/[#199](https://github.com/TheurgicDuke771/DataQ/issues/199)/[#204](https://github.com/TheurgicDuke771/DataQ/issues/204)/[#205](https://github.com/TheurgicDuke771/DataQ/issues/205), and backend [#194](https://github.com/TheurgicDuke771/DataQ/issues/194)/[#195](https://github.com/TheurgicDuke771/DataQ/issues/195). See [docs/progress.md](docs/progress.md).
