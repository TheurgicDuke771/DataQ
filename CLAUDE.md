...
**Next milestone:** **Week 7 — Deployment, hardening & docs (in progress).** Exit gate: production-ready v1 deployed to Azure, CI/CD live, team onboarded. **Cloud deploy is DONE** (ADR 0024/0025 — app live on ACA + SWA; the old RP-registration blocker is cleared: applied as subscription Owner, the 4 RPs registered, infra applied). **Remaining:** Azure Monitor→ADF webhook alert rule (have the live API URL now), the **FastMCP server (8 tools)**, the security hardening pass (Postgres VNet-private + KV purge-protection toggles already in TF), and team onboarding docs. **Done (2026-06-28):** the one-time **GHCR package→repo connect** (Actions-access grant — CI's `GITHUB_TOKEN` now pushes; no API exists for it, hence the one-time UI step) and the **end-to-end Deploy-pipeline validation** — the first full `workflow_dispatch` run surfaced two `deploy.yml` bugs (migrate job ran uvicorn not `alembic upgrade head` #401; frontend `pnpm/action-setup` version unset #402), fixed in #403, and the re-run went green through build→push→migrate→API/worker roll + SWA on `v6`.
