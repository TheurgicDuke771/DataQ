...
**Next milestone:** finish Week 4 frontend — check **dry-run preview** + **column-profiler panel** (next slice; backends ready), then **sharing/admin UI** + suite export/import UI, Monaco custom-SQL editor, and the bulk **connection health page**. The run-enablement backend the Results page (PR-C1) gates on is now complete (#215 PR-C0a + the run/result read API PR-C0b); PR-C1 is the in-app **Results page** + ADR 0018 (Grafana-deferral).
