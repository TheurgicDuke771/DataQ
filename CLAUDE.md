...
**Current week:** **Weeks 1–4 complete** (exit gates met). Week 4 done (**26/26**) — the admin page shipped (#289) cleared the last item. **Next: Week 5 — execution engine + scheduling** (early-credit already landed; see below).
...
**Next milestone:** **Week 5 — execution engine + scheduling.** Early-credit already landed in Week 4: worker runner-dispatch (#146), ADF/Airflow 10-min polling (#171), `trigger_bindings` CRUD (#172), per-suite run target + dispatch ungate (#215), manual run + run/result read API (PR-C0b). Remaining Week-5 surface: scheduled/cron runs, run lifecycle hardening, alerting wiring. Live warehouse/file run remains the deferred Week-1 smoke.
