...
**Current week:** **Week 5 — execution engine + scheduling, in progress (13/18)** (Weeks 1–4 complete, exit gates met). Phase 1–3 shipped: `error` status (#297), flat-file batch + `skip` (#298), UC run-path coverage (#299), run progress API (#301), cancel run + #227 dedup (#302), `GET /orchestration/pipelines` (#305), gap-recovery beat (#307) — on top of the Week-4 early-credit. **Remaining:** scheduling backend, execution UI (run-now / live-progress / scheduled / recent-runs), run-history retention.
...
**Next milestone:** **finish Week 5 — execution engine + scheduling** (13/18 done). Shipped: run paths across all datasource types (Snowflake + flat-file batch #298 + UC #299), `error`/`skip` operational statuses (#297/#298), run progress API (#301), cancel run (#302), provider-agnostic pipeline-status read `GET /orchestration/pipelines` (#305), gap-recovery beat #307, plus the Week-4 early-credit (worker dispatch #146, ADF/Airflow polling #171, `trigger_bindings` #172, target+dispatch #215, manual run + read API PR-C0b). Remaining Week-5 surface: **scheduling backend (A7)**, execution UI (run-now / live-progress / scheduled-runs / recent-runs), run-history retention purge. Live warehouse/file run remains the deferred Week-1 smoke.
