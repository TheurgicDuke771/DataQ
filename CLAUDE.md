...
**Current week:** **Week 6 of 8 — Results dashboard + alerting — IN PROGRESS (16/27)** (Weeks 1–5 complete, all exit gates met). Week-6 shipped so far: Results page scaffold + severity badges (PR-C1), Enhanced Monitoring Dashboard + run-detail route (PR #333), results filter bar + orchestration 30s-poll & DQ-run correlation (PR #347), **drawer→page restructures** (PR #350 — New-Suite page, edit flows → pages, New-Connection source-select split; 3 edit drawers removed), and layout/prototype-fidelity polish (PR #353 — centering `Page` wrapper, card grids, datasources-first source picker with per-category notes, Results KPI cards + Pipeline-runs "Provider run" column). Week-5 shipped: `error` status (#297), flat-file batch + `skip` (#298), UC run-path coverage (#299), run progress API (#301), cancel run + #227 dedup (#302), `GET /orchestration/pipelines` (#305), gap-recovery beat (#307), **scheduling backend (A7)** + **scheduled-runs UI (A6)**, **live run-progress UI (A3)**, **run-history retention sweep (A8)**, **run-now panel** — on top of the Week-4 early-credit. Closeout cleanups: profiler `null_fraction` DRY (#147) + `RUN_BAR_STATUS` refactor (#317); profiler N+1 perf split to backlog #327. The roadmap's recent-runs audit table was **moved to Week 6** (built once on the prototype `/dashboard`, not a gate item).
...
**Next milestone:** **Finish Week 6 — Results dashboard + alerting** (16/27 done). Phase 1–4 shipped (see "Current week" above). **Remaining (11 tasks):** the 6 alerting tasks (`ResultPublisher` seam + Teams notifications / suppression / dedup / severity-aware routing) and prototype Phase 5–6 — **Profile content** (`/profile`, `ProfileScreen`), **Workspace Settings** (`/settings`, `SettingsScreen`), **Admin layout-reconcile** (reconcile existing `/admin` page to prototype MetricCards + Members+access table layout), and **standard error pages** (4xx/5xx `ErrorState` catalog). ADR [0022](docs/adr/0022-week6-prototype-adoption-and-chart-library.md) scope: full 13-screen set as dedicated pages; connection edit drawers removed (edit flows are now pages); version-history + Share drawers remain; source picker is datasources-first, Orchestration last; recharts lazy-loaded. Live warehouse/file run remains the deferred Week-1 smoke.
**Active blockers:** none. Open follow-ups: profiler N+1 batching [#327](https://github.com/TheurgicDuke771/DataQ/issues/327) (backlog, split from #147), Week-4 frontend nits [#197](https://github.com/TheurgicDuke771/DataQ/issues/197)/[#199](https://github.com/TheurgicDuke771/DataQ/issues/199)/[#204](https://github.com/TheurgicDuke771/DataQ/issues/204)/[#205](https://github.com/TheurgicDuke771/DataQ/issues/205) (#192 nav prefix-match closed), Week-6 follow-ups [#348](https://github.com/TheurgicDuke771/DataQ/issues/348) (results env/datasource filter gap) / [#349](https://github.com/TheurgicDuke771/DataQ/issues/349) (runs fetch dedup) / [#351](https://github.com/TheurgicDuke771/DataQ/issues/351) (Test Connection draft endpoint) / [#352](https://github.com/TheurgicDuke771/DataQ/issues/352) (dashboard KPI enrichment), and backend [#194](https://github.com/TheurgicDuke771/DataQ/issues/194)/[#195](https://github.com/TheurgicDuke771/DataQ/issues/195). See [docs/progress.md](docs/progress.md).
