...
**Next milestone:** finish Week 4 frontend — check **dry-run preview** (PR-D1; `DryRunPreview`), the **column-profiler panel** (PR-D2; `ColumnProfilePanel` in the create page + edit drawer, on `POST /suites/{id}/profile`), and the **suite export/import UI** (PR-E1; `ImportSuiteDrawer` + `utils/download.ts` Export action, on `GET /suites/{id}/export` · `POST /suites/import`) shipped; next the **sharing/admin UI** — its user-directory backend is now in (PR-E2: `GET /users/search` behind a FastAPI-free `user_service` + `ShareRead` enriched with the grantee's `email`/`display_name` via a joined `Share.user`), so **PR-E3** builds the panel itself — plus the Monaco custom-SQL editor and the **Suite Triggers** UI ([#216](https://github.com/TheurgicDuke771/DataQ/issues/216)).
