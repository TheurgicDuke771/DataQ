...
> Developers working on DataQ itself can run with no sign-in at all: `DATAQ_SIGNIN_EMAIL= DATAQ_DEV_BYPASS=true DATAQ_AUTH_MODE=bypass docker compose -f docker-compose.ghcr.yml up` — every request becomes one fixed admin user. It is off by default and never a fallback; omitting `DATAQ_SIGNIN_EMAIL` entirely stops the stack instead of choosing that for you.
