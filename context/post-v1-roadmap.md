...
- [Security & data handling](../docs/site/security/overview.md) (now carries its own Compliance section) + [docs/site/compliance/](../docs/site/compliance/) — GDPR / CCPA-CPRA / HIPAA technical controls (published, long-term reference); ~~compliance-posture.md~~ — **deleted 2026-08-28**: it was the internal gap-tracking doc these were built from, G1–G6 all shipped (the #1460 tamper-evidence residual closed 2026-08-27) bar the accepted-exception #1385, and its content was folded into `docs/site/security/overview.md`
...
| [#1702](https://github.com/TheurgicDuke771/DataQ/issues/1702) | **Admin control-centre epic (filed 2026-08-29)** — the successor home for this theme's admin surface: routed sub-pages, in-app membership ([#1693](https://github.com/TheurgicDuke771/DataQ/issues/1693)), the write pass ([#1698](https://github.com/TheurgicDuke771/DataQ/issues/1698), supersedes #412), admin lens ([#1700](https://github.com/TheurgicDuke771/DataQ/issues/1700), supersedes #411), health page + parity pass. Design + mockup in the epic. |
...
**Design pinned + issues FILED 2026-07-10 (the #596 deliverable, pulled forward to v1.1 W3):**
the full design lived in docs/post-v1-assets-lineage-incidents-notes.md (deleted at the
v1.1 close — everything shipped; ADR 0034 is the durable record: asset identity = OpenLineage
naming verbatim; lineage emitted/pulled, never built; residual = #1515 owner routing).
Filed: **#757** (asset entity — phase 1, the prerequisite), **#758** (OpenLineage emission),
**#759** (dbt manifest → `lineage_edges`), **#760** (asset view — phase 2), **#761** (incident
objects), **#762** (`LineageProvider` + Marquez). Phase 4 (nav inversion) files after #760 proves
the asset page.
...
at-rest encryption) — see [Security & data handling](../docs/site/security/overview.md)
...
| [#1662](https://github.com/TheurgicDuke771/DataQ/issues/1662) | **Generic HMAC-signed outbound-webhook `ResultPublisher`** — one publisher makes the alerting side vendor-neutral: signed JSON POST (mirroring the Airflow *ingest* signing pattern), payloads pass the same redaction rules. Prerequisite: the #389 channel-neutral rename *(closed — unblocked)*. |
...
_Design pinned 2026-07-10 (ADR 0034 + the #596 doc, since deleted at the v1.1 close — shipped): asset entity = #757; OL emission w/
