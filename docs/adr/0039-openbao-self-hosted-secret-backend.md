...
**6. Failure modes stay distinguishable.** `get`/`set` keep raising `SecretNotFoundError` / `SecretWriteError`, preserving the Protocol contract every caller already handles — but **403 and 503 additionally emit a WARNING log** (`openbao_unreachable` / `openbao_permission_denied`) carrying the status, and the exception message states the cause. Per decision 4 of ADR [0034](0034-asset-entity-openlineage-identity-lineage-pull.md)'s reasoning about observations vs outages: an operator whose vault is sealed or whose token expired must be able to find that out from a log line, not from inferring it out of "credential missing" on every connection at once.
