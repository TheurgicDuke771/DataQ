...
| [0015](0015-two-connection-comparison-check-model.md) | Two-connection comparison check model — suite stays single-connection and supplies the **target under test**; a `comparison` check adds one **source** ref (`checks.source_connection_id` FK, RESTRICT + 409, kind⇔presence CHECK) + suite-target-shaped `config.source` validated by the same resolver (either side may be a read-only SQL query per 0019); new `DatasetReader` seam + ported FDC engine (batched-read latitude), row-cap fail-fast (no silent truncation); mismatch-% → `metric_value` under 0016 banding; report files derived on-demand from redacted buckets, never stored; side-by-side editor (target connection locked to the suite's); **no** connection→connection generalization (0030 Option B stays deferred) | Accepted |
