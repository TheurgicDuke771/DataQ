...
1. **One new table, `workspace_members`** — `id`, `email` (normalized, with a unique index over `lower(email)` mirroring `uq_users_email_lower`; there is one normalization rule in this codebase and it is `otp_service.normalize_email`), `initial_role`, `invited_by` (FK → `users.id`), `created_at`. No FK to `users`: a member is admitted **before** any user row exists, and the row must outlive an offboarded user. Additive migration, no change to `users`.
...
4. **Four choke points, named, each a real single point of resolution:**
   ...
   Denial is **403 with a valid-credential message**, never 401 — the rule the generic-OIDC signup gate already follows: the token *is* valid, so a 401 loops the SPA through sign-in forever.
...
8. **Three lockout guards, because the failure mode here is locking the workspace out of itself.**
   - **The first row write auto-imports every existing `users` row as a member, in the same transaction.** Turning enforcement on can never evict a current user. Not a migration and not a background job: it is part of the first insert, or it is a race.
...
- **A migration that populates the table (non-empty by default)** — rejected. It converts a docs-only, zero-risk deploy into one that turns on enforcement everywhere at once, on data backfilled from `users` — and a `users` row is not evidence of intended membership (it is evidence somebody once signed in). The auto-import on first write gets the same effect at the moment an admin has actually decided to switch.
...
- Superuser blindness applies as ever: tests run as an owner that bypasses `REVOKE`, so anything resting on a privilege needs the real-privilege harness — a superuser bypasses `REVOKE`, and the append-only audit table already produced one production incident that way.
