...
| 9 | **Outbound LLM intelligence** — DataQ calling a model on its own behalf | Schema + masked profiler statistics for SQL generation and check suggestions; RCA narratives also send the triggering check's own observed/expected values, not yet masked. Never raw sample rows | Potentially — profiler statistics, or (RCA only) an actual observed value from the check; model provider and jurisdiction are admin-chosen | **Live, off by default** — an admin must configure a provider + credential before any call leaves |
