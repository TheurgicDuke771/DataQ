...
| ✅ | [#1632](https://github.com/TheurgicDuke771/DataQ/issues/1632) Prompt-injection adversarial battery for #1513/#1512 — extends `tests/support/adversarial.py` with a shared injection-string battery; proves a hostile column/top-value/table/schema string reaches the prompt as inert data (never bypasses the output gate), a structural guarantee that no raw sample row is ever readable by either builder, and that the model's response is never logged unredacted (mutation-verified — all three checks fail against a deliberately reintroduced leak) | intelligence |
