...
| **Open issues** | **54** open repo-wide (2026-08-29) — **W3's LLM track opened and landed its core the same day**: ADR 0042 Accepted (#1636), the `LLMProvider` seam backend merged (#1640, closes #1511), #1247 (#1637) and #1307 (#1638) closed, and the RCA-survey find #1634 (evidence card dark in the UI) closed by #1639. New filings from the research/RCA pass: #1631 (Ollama live lane), #1632 (prompt-injection battery), #1633 (LLM RCA narrative, W4), #1635 (evidence-card enrichment, backlog). Still open from W1: #590/#588 (user decision gates) + #1392/#1257. |
...
| **Open PRs** | **1** (2026-08-29): [#1641](https://github.com/TheurgicDuke771/DataQ/pull/1641) — the #1511 admin LLM settings panel (merge after its CI rerun on the updated branch). ⚠️ Undeployed on main: the #1624 pushdown batch + the whole LLM seam (migration `dd652ae1ef85` in the next deploy's delta). |
