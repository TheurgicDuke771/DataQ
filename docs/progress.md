...
| ✅ | [#1515](https://github.com/TheurgicDuke771/DataQ/issues/1515) Incident/alert "Owner" attribution prefers the asset owner when set, falls back to suite owner: `alerting/builder.py` resolves the run's asset the same way `_asset_column_tags` already does (`run.asset_id` then `suite.asset_id`), and if that asset has `owner_user_id` set, uses that user instead of `suite.created_by` — display-only (Teams card / Slack message / email "Owner" row); delivery routing itself is unchanged. Regression tests mutation-verified: the "prefers asset owner" test genuinely fails against the unfixed one-liner, the fallback (no asset owner) case is unchanged | incidents |
