...
| ✅ | [#1648](https://github.com/TheurgicDuke771/DataQ/issues/1648) Cadence-aware suggestions — bound-pipeline schedule + delay history drive thresholds: `orchestration_service.compute_pipeline_cadence` (median/max inter-run gap from `pipeline_runs`, insufficient-history honesty, a deterministic margined-default threshold with no LLM involved) grounds an optional `monitor:freshness` suggestion only when an enabled trigger binding exists and its cadence is grounded — never offered ungrounded. Reuses the existing #1199 near-miss machinery as a deterministic `coverage_warnings` field, computed independently of what the model returns. The cadence computation is UI-agnostic and ready for a check-editor hint; that hint itself, like #1512/#1513's own review/apply UI, ships next | intelligence |
