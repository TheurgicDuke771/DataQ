...
| **Next milestone** | Week 4 (frontend) nearly done: connection manager + suites list/detail + catalog check editor + dry-run + profiler + export/import + **sharing panel** + **run-target editor** + **manual Run button** + **Suite Triggers panel** (#216) all shipped; **remaining**: admin page + Monaco custom-SQL (both **backend-blocked** — need list-all + a custom-SQL check kind) + the check version-history drawer. Week-5 early-credit landed (worker runner-dispatch [#146](https://github.com/TheurgicDuke771/DataQ/issues/146), ADF/Airflow polling [#171](https://github.com/TheurgicDuke771/DataQ/issues/171), `trigger_bindings` CRUD [#172](https://github.com/TheurgicDuke771/DataQ/issues/172)) |
...
**Week 4 total: 22 / 26 ✅ (1 🔵)** _(connection manager UI complete (6/6); check editor core shipped + **dry-run preview** (PR-D1) + **column profiler panel** (PR-D2) + the datasource-shaped **run-target editor** (PR #246 — closes the flat-file + Unity Catalog target inputs, making suites runnable); **suite export/import UI** (PR-E1); **suite sharing panel** (PR-E2 backend + PR-E3 UI, gated on `SuiteRead.my_permission`); **GX-Cloud redesign complete (4/4)** — classified connection + check create pages, edit-only drawers, suite-route/categorized picker (PR-A1/A2/B1/B2); PR-3c polish 2/3 (Settings `forbid` 🔵 blocked on the `.env` split) — **admin page** + **Monaco custom-SQL** remain (both backend-blocked), plus the check version-history drawer)_
...
### Prototype adoption — dedicated-page flows + Dashboard landing (added — not in original roadmap) (6 tasks — 0/6)
...
### Prototype adoption — backend-gated screens + final polish (added — not in original roadmap) (5 tasks — 0/5)
