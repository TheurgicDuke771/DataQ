...
| **Open issues** | **66** (at planning, 2026-07-04): **27 scheduled** onto the `v1.1 Week 1..6` milestones (incl. #587–#596 filed at planning from design-captured items) + the cycle epic [#597](https://github.com/TheurgicDuke771/DataQ/issues/597) + **38** remaining on `Backlog (post-v1 / testing)` — mapped by theme in [post-v1-roadmap.md](../context/post-v1-roadmap.md), not duplicated here |
| **Open PRs** | none |
...
> not scheduled below stays themed on the `Backlog (post-v1 / testing)` milestone.
>
...
| ⬜ | [#492](https://github.com/TheurgicDuke771/DataQ/issues/492) ADF webhook live delivery — **final decision**: last-window attempt or close as won't-do-without-Azure (poll fallback remains the path of record) | Theme 5 |
| ⬜ | [#590](https://github.com/TheurgicDuke771/DataQ/issues/590) Azure wind-down: G-i harness teardown, `terraform destroy`, credential retirement, state disposition | ops / G-i |
...
### v1.1 W5 — `anomaly` monitor kind + metric trends (due 2026-08-08) — 0/3
...
### v1.1 W6 — scale-aware execution + hardening + cycle close (due 2026-08-15) — 0/7
...
### Deliberately NOT scheduled this cycle

Stays on Backlog by choice: Theme 2 LLM authoring/RCA (needs the `LLMProvider` seam design,
follows G-d), Theme 3 asset entity + asset-first IA (its phase 1 is what the #596 design doc
files for the *next* cycle), the Theme 8 adapter expansion (PG adapter is the natural next-cycle
opener — dogfoodable locally), Theme 4 compliance, #505 AWS/GCP IaC (no target cloud
subscription yet — becomes the re-deploy path after #590), and everything else themed in
