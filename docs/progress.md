...
| **Current week** | **Week 5 of 8 (Execution engine + scheduling)** — in progress (11/18); Weeks 1–4 complete (26/26) |
...
| **Next milestone** | **Week 5 in progress (11/18)** — async-execution backend: `error` status (#297), flat-file batch + `skip` (#298), UC run-path coverage (#299), run progress API (#301), cancel run + #227 dedup (#302) all shipped on top of the early-credit (Celery runner PR 4, worker dispatch #146, ADF/Airflow polling #171, `trigger_bindings` #172, target+dispatch #215, manual run + read API PR-C0b, Suite Triggers UI #216). **Remaining:** scheduling backend (A7), execution UI (run-now / live-progress / scheduled-runs / recent-runs), gap recovery + `GET /orchestration/pipelines`, run-history retention purge. |
...
**Week 5 total: 11 / 18** _(early credit: Celery task runner in PR 4; worker runner-dispatch #146; ADF + Airflow 10-min polling #171; plus `trigger_bindings` CRUD #172 out-of-band; check target-table + dispatch ungate #215 (PR-C0a); manual run trigger + run/result read API (PR-C0b); **Suite Triggers UI #216 (PR #247)**. **This stretch (Phase 1–2):** `error` status #297, flat-file batch + `skip` #298, UC run-path coverage #299, run progress API #301, cancel run + #227 #302. +3 tasks re-tracked here: check target-table #215 (slipped from W3), Suite Triggers UI #216, and the run-enablement read API (PR-C0b). The **manual Run button** (PR #245) + the **run-target editor** (PR #246) also shipped earlier this week. **Remaining (7):** run-history retention, gap recovery, `GET /orchestration/pipelines`, run-now panel, live run-progress UI, scheduled-runs table, recent-runs audit table — plus the scheduling backend (A7))_
