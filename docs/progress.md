...
| **Current week** | **Week 5 of 8 (Execution engine + scheduling)** — in progress (13/18); Weeks 1–4 complete (26/26) |
| **Roadmap tasks done** | 83 ✅ + 18 🟡 / 180 (~46%) |
...
| **Week-5 exit gate** | Async runs with live progress across all datasource types; scheduling operational. — **partially met**: run paths across all datasource types done (Snowflake + flat-file batch + UC), run **progress** + **cancel** APIs shipped, `error`/`skip` operational statuses landed. Remaining: scheduling backend + execution UI + retention purge + orchestration `/pipelines` + gap recovery. Weeks 1–4 exit gates met — see [CLAUDE.md §13](../CLAUDE.md). Live warehouse/file run remains the deferred Week-1 smoke. |
| **Next milestone** | **Week 5 in progress (13/18)** — async-execution backend: `error` status (#297), flat-file batch + `skip` (#298), UC run-path coverage (#299), run progress API (#301), cancel run + #227 dedup (#302), `GET /orchestration/pipelines` (#305), gap-recovery beat (#307) all shipped on top of the early-credit (Celery runner PR 4, worker dispatch #146, ADF/Airflow polling #171, `trigger_bindings` #172, target+dispatch #215, manual run + read API PR-C0b, Suite Triggers UI #216). **Remaining:** scheduling backend (A7), execution UI (run-now / live-progress / scheduled-runs / recent-runs), run-history retention purge. |
...
| **Open PRs** | [#302](https://github.com/TheurgicDuke771/DataQ/pull/302) (A2 cancel run) — in review |
...
### Async execution backend (9 tasks — 8/9 ✅)
...
**Week 5 total: 13 / 18** _(early credit: Celery task runner in PR 4; worker runner-dispatch #146; ADF + Airflow 10-min polling #171; plus `trigger_bindings` CRUD #172 out-of-band; check target-table + dispatch ungate #215 (PR-C0a); manual run trigger + run/result read API (PR-C0b); **Suite Triggers UI #216 (PR #247)**. **This stretch (Phase 1–3):** `error` status #297, flat-file batch + `skip` #298, UC run-path coverage #299, run progress API #301, cancel run + #227 #302, `GET /orchestration/pipelines` #305, gap-recovery beat #307. +3 tasks re-tracked here: check target-table #215 (slipped from W3), Suite Triggers UI #216, and the run-enablement read API (PR-C0b). The **manual Run button** (PR #245) + the **run-target editor** (PR #246) also shipped earlier this week. **Remaining (5):** run-history retention, run-now panel, live run-progress UI, scheduled-runs table, recent-runs audit table — plus the scheduling backend (A7))_
...
### Consistency & transaction hardening (added — from the ACID/consistency review) (4 items — 0/4)
...
- [ ] ⬜ **History/audit strategy (ADR)** — check-version history is lost on delete (`check_versions.check_id` CASCADE); no general audit log; no soft-delete; no SCD-2. Decide the posture (preserve-on-delete / soft-delete / audit-log / SCD-2). [#310](https://github.com/TheurgicDuke771/DataQ/issues/310) _(SCD-2 cost analysis recorded on the issue)_
...
- [x] ✅ **Admin control centre page** (`/admin`) — **shipped early in Week 4 (#289)**. Suites / Users / Access tabs on the new endpoint, with a **real 403** `Forbidden` page for non-admins (server-driven via `/me`'s `is_workspace_admin` — *not* a client "View as" toggle), admin-only nav, shared `MeProvider`/`useMe`.
...
**Week 7 total: 2 / 34** _(+5 prototype-adoption tasks: workspace-admin endpoint ✅ + Admin page ✅ both shipped early in Week 4 via #289; remaining: Settings page, Dashboard KPI honesty pass, visual-fidelity pass)_
