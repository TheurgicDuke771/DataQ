...
| **Current week** | Week 4 of 8 (Connection manager UI + check editor UI) — **complete (26/26)**; **Week 5 next** (Weeks 1–4 complete) |
| **Roadmap tasks done** | 78 ✅ + 16 🟡 / 177 (~44%) |
...
| **Week-4 exit gate** | Users can configure any connection type and author checks end-to-end in the UI. — **met** (full connection manager + catalog/Monaco check editor + dry-run + profiler + run-target + export/import + sharing + version-history + **admin control centre #289**). Weeks 1–3 gates also met — see [CLAUDE.md §13](../CLAUDE.md). Live warehouse/file run remains the deferred Week-1 smoke. |
| **Next milestone** | **Week 4 complete (26/26)** — connection manager + suites list/detail + catalog check editor + dry-run + profiler + export/import + **sharing panel** + **run-target editor** + **manual Run button** + **Suite Triggers panel** (#216) + **Monaco custom-SQL editor** (ADR 0019; backend #258 + frontend #259) + **check version-history drawer** (#280) + **admin control centre** (#289 — workspace-admin authz + `/admin` endpoints + page/403) all shipped. Next: **Week 5 — execution engine + scheduling** (early-credit already landed: worker runner-dispatch [#146](https://github.com/TheurgicDuke771/DataQ/issues/146), ADF/Airflow polling [#171](https://github.com/TheurgicDuke771/DataQ/issues/171), `trigger_bindings` CRUD [#172](https://github.com/TheurgicDuke771/DataQ/issues/172)) |
