...
|---|---|---|
| **2026-09-06** | Snowflake `DATAQ_LOADER_PAT` | Re-minted 2026-08-22 (see Credential rotation table). The ADF `ls_snowflake` inline credential is still on the OLD value — needs a `-target` tofu apply (or an ADF Studio edit) before it lapses too. |
| **2026-09-06** | Snowflake **ACCOUNTADMIN PAT** | Re-minted 2026-08-22 into harness `secrets.sh` only — nothing runs on it day to day, so re-mint again only if a `tofu apply` is needed before it lapses. |
| **2026-09-06** | Snowflake `DATAQ_READER_PAT` | Re-minted 2026-08-22 (see Credential rotation table). **Not yet connection-tested against the live API** — no working non-interactive PAT was available this session (both `dataq-pat-w1-admin`/`dataq-pat-w1-member` are dead); confirm the 3 prod Snowflake suites run green. |
