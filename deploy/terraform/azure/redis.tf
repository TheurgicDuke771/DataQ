...
# Defense-in-depth: even though ingress is internal-only (reachable solely from within this
...
resource "azurerm_container_app" "redis" {
  ...
  template {
    ...
    container {
      name = "redis"
      # 7.x is a deliberate pin, not staleness: Redis 8 bundles the Vector Sets module, whose
      # duplicate-HNSW-ID RESTORE path is an unpatched RCE.
