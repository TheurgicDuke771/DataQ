...
| `RATE_LIMIT_AUTHENTICATED_PER_MINUTE` / `RATE_LIMIT_UNAUTHENTICATED_PER_MINUTE` / `RATE_LIMIT_WEBHOOK_PER_MINUTE` | `300` / `120` / `30` | per-minute caps, keyed per `sha256(bearer)` (authenticated) or client-IP (rest). Defaults are generous; **tighten `RATE_LIMIT_WEBHOOK_PER_MINUTE`** to your orchestrator's real callback cadence. |
