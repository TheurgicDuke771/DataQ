...
| `UC_SQL_PUSHDOWN` | `true` | Unity Catalog runs the audited catalog expectations on the Databricks SQL Warehouse (worker memory flat, #1532). `false` restores the pre-#1532 read-into-pandas routing wholesale — the rollback lane for a dialect surprise. |
