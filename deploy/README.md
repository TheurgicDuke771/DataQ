...
| `RUN_ADMISSION_BUDGET_BYTES` | `1073741824` (1 GiB) | the budget the two caps above do **not** provide: a per-**worker-container** ceiling on how much dataset its prefork children may hold *at once*, claimed in Redis before a run materialises anything and released when it finishes. `RUN_MAX_SCAN_BYTES` bounds one run; four overlapping 1M-row flat-file runs each passing it still wanted ~3.1 GiB against a 2 GiB worker (`docs/site/architecture/perf-baseline.md`). 1 GiB = `RUN_MAX_SCAN_BYTES` × the CSV expansion factor, i.e. exactly one at-the-cap full read in flight. A run that does not fit is **re-queued, never failed**, and reports `queued_reason: awaiting_worker_memory` on `/runs` and `/runs/{id}/progress` so a waiting run is not mistaken for a running one. Pushdown lanes (Snowflake, and Unity Catalog when every expectation pushes down) hold no dataset and bypass this entirely; sampled runs are estimated from the sample, not the object. **Fails open** — an unreachable Redis admits and logs `run_admission_store_unavailable`, per the ADR 0035 stance. `0` disables admission control completely. |
