...
| `RATE_LIMIT_AUTH_PER_MINUTE` | `10` | strict per-IP cap for `/api/v1/auth/*` (OTP mint/verify, #1127) — checked **before** the bearer branch, so a token can't dodge it. This is the per-IP half only; the per-email counters (ADR 0032 §8) land separately with #734. |
