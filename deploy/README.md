...
| `AUTH_SESSION_COOKIE_SECURE` | unset (inferred) | leave **unset** in any normal deployment: `Secure` is inferred from `X-Forwarded-Proto`, which the frontend nginx sets. Hard-coding `true` breaks plain-HTTP local dev *silently* (the browser accepts the `Set-Cookie` and then never sends the cookie back). Set explicitly only if your proxy does not forward that header. |
