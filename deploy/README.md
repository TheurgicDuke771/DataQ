...
| `RUN_MAX_SCAN_ROWS_ICEBERG` | *(unset — inherits `RUN_MAX_SCAN_ROWS`)* | the same row guardrail for the **Iceberg** runner, which materialises the whole current snapshot (`scan().to_arrow()`) for both its expectations and its freshness scan-fallback. Probed with `scan().count()`, which is snapshot metadata and reads no data files. Iceberg has its **own knob** because its measured ceiling is not Unity Catalog's — UC passed 1M and OOM-killed at 2M, Iceberg passed 2M — so one shared number would refuse a rung Iceberg is measured to survive. Leave it unset to inherit; set it to the Iceberg-measured value. `0` disables the cap *and* the probe. Iceberg targets **cannot** be sampled (the spec is refused at save time), so the over-cap message says "narrow the target", not "sample". |
