...
- **Frontend:** React + Vite + Ant Design (antd v6), Monaco, recharts. Routes are deep-linkable pages (ADR 0022): `/dashboard`, `/assets` + `/assets/:assetId` (the primary lens since the ADR 0034/0037 nav inversion), `/connections`, `/connections/new`, `/connections/:connectionId/edit` (both **role-gated**: wrapped in `RequireRole minimum="admin"` — a Member/Viewer deep-linking there must get the Forbidden state, not the form; ADR 0033 #743), `/suites`, `/suites/:id`, `/suites/new`, suite edit, check editor + check edit under a suite, `/results` (Runs + Pipeline runs tabs), run detail, `/incidents` surfaces via the asset page, `/profile`, `/settings`, `/admin` (incl. Members role editor), plus the 404 page. `frontend/src/pages/` is the authoritative list — check it for screens added since this file was written.
- **Backend:** FastAPI at `/api/v1/*`; the frontend calls it via `frontend/src/api/*.ts` (axios). Datasource vs orchestration is load-bearing (CLAUDE.md §4): Snowflake / ADLS Gen2 / S3 / Unity Catalog / Iceberg are datasources; ADF / Airflow / dbt are orchestration providers (never checkable datasources).
- **Local run:** the Playwright config (`frontend/playwright.config.ts`) serves the app at `http://localhost:3000` via `pnpm dev --host --port 3000`, with **auth dev-bypass on** (identity `dev-bypass@dataq.local`, a workspace admin), so every route loads without a login. Reuse a running `:3000` server if present; otherwise start one (`cd frontend && pnpm dev --host --port 3000 &`) and wait for it. Confirm the backend is up (`docker compose ps` — services have no fixed `container_name`; the stack also carries `openbao`, `mailpit`, and optional `marquez`/`jaeger` profiles; the API is proxied same-origin) — if the backend is down, say so and scope to render-only checks.
