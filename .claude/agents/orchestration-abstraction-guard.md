...
1. **Any switch on provider identity** outside the orchestration package. Includes ALL of the following forms — be exhaustive, not just `==`:
   ...
   if provider in ("adf", "airflow", "dbt"):  # FORBIDDEN
...
3. **Webhook routes that bypass the agreed shape.** ADR 0004 (extended to dbt by ADR 0029) defines explicit sibling routes `POST /api/v1/orchestration/events/adf`, `.../airflow` and `.../dbt` (the trailing path segment names the provider; it is not a `{provider}` capture). A route like `/api/v1/adf/events` — provider name outside the `/orchestration/events/` namespace — is the violation. The implemented `/api/v1/orchestration/events/adf` that resolves the provider via `get_orchestration_provider("adf")` is correct.
...
4. **`pipeline_runs` queries with a hardcoded provider literal baked into shared logic:**
   ...
   ```
   Provider values are plain strings validated against `ORCHESTRATION_PROVIDERS` (`backend/app/db/models.py` — TEXT + CHECK, deliberately **not** a Python/PG enum). The smell is a literal `"adf"`/`"airflow"`/`"dbt"` hardcoded into provider-agnostic code, not the use of `str` itself.

### 🟡 Yellow flags (call out, don't necessarily block)

1. **A literal provider string** (`"adf"`, `"airflow"`, `"dbt"`) appearing in `backend/app/services/` or `backend/app/api/` outside the registry-resolution boundary, rather than the value flowing from `ORCHESTRATION_PROVIDERS` / a resolved provider. (Provider is a `str` by design — flag the hardcoded literal, not the type.)
2. **`TODO` / `FIXME` / `XXX` comments mentioning ADF, Airflow or dbt** — may indicate deferred work that should be tracked as an issue per working-agreement #3.
3. **Test data fixtures that only cover ADF**. All three providers (adf / airflow / dbt) should be exercised; single-provider coverage suggests the abstraction has rotted.
4. **`trigger_bindings` rows hardcoded to a single provider** in seed data or fixtures — should demonstrate more than one of the three providers.

### 🟢 Acceptable patterns

- Provider-specific code **inside** the orchestration package — `backend/app/orchestration/adf.py`, `airflow.py`, `dbt.py`, `base.py`, `registry.py` (file-per-provider; that's where it belongs).
...
- **Test code may import concrete provider implementations** (`AdfProvider`, `AirflowProvider`, `DbtProvider`) for isolation testing — the abstraction rule binds production code (`backend/app/services/`, `backend/app/api/`), not tests.

## False positives to avoid

Don't flag these — a provider name appearing in source is not automatically a branching violation:

- **String literals inside docstrings or comments** — e.g. a comment `# all three providers POST here` or a docstring listing supported providers. These are documentation, not branching.
- **`@pytest.mark.parametrize` fixtures that enumerate providers** — e.g. `@pytest.mark.parametrize("provider", ["adf", "airflow", "dbt"])`. Enumerating the providers in a test is *good* coverage (the opposite of the ADF-only smell), not a hardcoded-literal violation.
...
- **`provider == provider` / value flowing from `ORCHESTRATION_PROVIDERS`** — comparing against a passed-in/resolved provider value is fine; only a hardcoded `"adf"`/`"airflow"`/`"dbt"` *literal* in provider-agnostic code is the smell.
