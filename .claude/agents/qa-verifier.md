...
10. **MCP surface** (ADR 0008 — 46 tools at `/mcp/`, trailing slash) — the same service layer over a second protocol. Gate truth lives in `backend/tests/support/mcp_gates.py` (`GATES` declares each tool's authz as data; the 46 split 23 read-only / 18 state-changing / 5 live-probe-gated-like-writes). Spot-check with a PAT: a read tool works for a `view`-granted user, a mutating tool 403-equivalents for them, and NO tool performs a connection mutation (no admin-only MCP tool exists by design — a credential must never transit an LLM). Under email-OTP mode only a PAT authenticates `/mcp` (a session cookie must be rejected, #1151).
