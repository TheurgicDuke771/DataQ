...
**Not in the tables but still CI merge gates:** dependency audits (`backend-audit`/`frontend-audit` — run via the `/security-scan` skill, or lift CI's exact invocations from ci.yml (no `--ignore-vuln` flags since #553)), CodeQL (CI-only), and betterleaks secret scanning (the pre-commit hook covers it locally). A green table here does NOT clear those — say so in the report. If these tables and `.github/workflows/ci.yml` ever disagree, ci.yml wins; flag the drift as a finding.
...
5. **Authz probes** — authorization is TWO axes (ADR 0033): the stored workspace role (`users.role`: admin/member/viewer) × per-suite `view`/`edit` grants. With seeded demo users verify both: a `view`-granted user 403s on edit endpoints; a non-shared suite is invisible in lists AND 403/404 by direct id (no IDOR); `/admin/*` 403s for non-admins; **connection create/update/delete/reauth and the draft `POST /connections/test` 403 for Members** (Admin-only since #741); a **Viewer** cannot author, cannot receive an `edit` share (capped at `view` even for a pre-existing grant), and cannot open an outbound connection test. NOTE: the dev-bypass principal IS a workspace admin (#741) — it cannot stand in for a non-admin; use the seeded member/viewer users (the `as_role` fixture pattern in tests).
...
7. **Webhook hostility** — POST to `/api/v1/orchestration/events/<provider>` for every provider in `ORCHESTRATION_PROVIDERS` (`backend/app/db/models.py`) with: missing/wrong auth (secret/HMAC per provider), valid auth + malformed JSON, valid JSON missing required fields, duplicate delivery (dedup index #456 should absorb it). Expect 401/422 envelopes and no phantom `pipeline_runs` rows. **Local prerequisite:** nothing seeds the per-provider webhook secrets into the local secret store — the local store is **OpenBao** (ADR 0039; compose service `openbao`, the plaintext Redis store is deleted). Plant scratch values yourself first (`docker compose exec -e BAO_ADDR=http://127.0.0.1:8200 -e BAO_TOKEN="$OPENBAO_TOKEN" openbao bao kv put secret/<name> value=<scratch>` — both env vars are required: the image sets no BAO_ADDR (the CLI defaults to HTTPS against the HTTP dev listener) and the CLI does not read BAO_DEV_ROOT_TOKEN_ID; the token is in the gitignored `.env`), or run only the missing/wrong-auth sub-cases and report the valid-auth ones as prerequisite-skipped, never as failures.
...
10. **MCP surface** (ADR 0008 — 47 tools at `/mcp/`, trailing slash) — the same service layer over a second protocol. Gate truth lives in `backend/tests/support/mcp_gates.py` (`GATES` declares each tool's authz as data; the 47 split 24 read-only / 18 state-changing / 5 live-probe-gated-like-writes). Spot-check with a PAT: a read tool works for a `view`-granted user, a mutating tool 403-equivalents for them, and NO tool performs a connection mutation (no admin-only MCP tool exists by design — a credential must never transit an LLM). Under email-OTP mode only a PAT authenticates `/mcp` (a session cookie must be rejected, #1151).
