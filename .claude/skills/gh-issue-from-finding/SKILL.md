...
- `--milestone <title>` — optional override. Every issue gets a milestone (project convention). Default: the current feature-week milestone from CLAUDE.md §13 (e.g. `v1.2 Week 1 — decision gates + MCP honesty burn-down`); use the current cycle's rolling backlog (`v1.2 Backlog`) for no-target-week items.
...
2. **Resolve and validate the labels.**
   ...
   - **Validate both labels exist before filing.** Run `gh label list --json name -q '.[].name'` and confirm each resolved label is present. If a label is missing, stop and tell the user (e.g. `gh issue create` with an unknown `--label` fails with "label not found") — do not invent or create labels. The current repo labels include: `bug`, `enhancement`, `documentation`, `refactor`, `test`, `ci`, `ops`, `chore`, `security`, `epic`, `dependencies`, `priority/P0`–`priority/P3` (plus GitHub defaults and `week-1/2-carryover`). A `--type` outside `bug|enhancement|documentation` (e.g. `refactor`/`test`/`ci`/`ops`/`chore`) is allowed only if that label already exists.
   - Pass both to `gh issue create` as a comma-joined `--label "$TYPE_LABEL,$PRIORITY_LABEL"`.

2a. **Resolve and validate the milestone.** Every issue gets a milestone. Resolve against the live list: `gh api repos/TheurgicDuke771/DataQ/milestones --jq '.[] | .title + " | " + .state'`. Default = the **open** milestone whose title starts with the current cycle's week prefix (`v1.2 Week <N> — ` as of the v1.2 cycle — read the prefix off the live list, not from memory of an older cycle) for the week CLAUDE.md §13 names — match against the live titles, do NOT transcribe §13's decorated headline (it carries a `(due YYYY-MM-DD)` suffix that is not part of the milestone title). No-target-week items → the open milestone ending in `Backlog` (currently `v1.2 Backlog`); note each cycle close CLOSES the previous backlog milestone, so never fall back to a closed one. If the user names an already-closed milestone (rare backfill): create the issue without `--milestone`, then attach it via the REST procedure in Rules. If nothing matches at all, stop and tell the user — do not create milestones and do not leave the issue milestone-less. Otherwise pass `--milestone "$MILESTONE"`.
...
- **Every issue gets a milestone** (in addition to labels). Feature-week milestone by default; the current cycle's `… Backlog` milestone for unscheduled items. Note: assigning to an already-closed milestone can't be done via `gh issue edit --milestone <title>` — use the REST API with the milestone *number* (`gh api -X PATCH repos/TheurgicDuke771/DataQ/issues/<N> -F milestone=<num>`).
