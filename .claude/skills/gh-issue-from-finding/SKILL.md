...
2a. **Resolve and validate the milestone.** Every issue gets a milestone. Resolve `--milestone` (or the default per the Usage section) against `gh api repos/TheurgicDuke771/DataQ/milestones --jq '.[].title'` (open milestones only). If it doesn't exist, stop and tell the user — do not create milestones and do not file milestone-less. Pass it as `--milestone "$MILESTONE"`.
